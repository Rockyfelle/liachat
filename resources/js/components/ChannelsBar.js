			...
			<div className="flex flex-col m-0 overflow-auto h-[87vh] pb-5 overflow-auto p-2 pr-0">
				...
				<div className="flex flex-row p-2 px-5 mb-2 rounded-l-lg cursor-pointer bg-orange-700 align-middle place-content-between"
					onClick={() => {
						...
						window.history.replaceState(null, '', `/program/${progs.programId}/assignments`);
				...
				{progs.channels.map((channel, index) => {
					return (
						<div
							key={"message" + index}
							className={`p-2 px-5 mb-2 rounded-l-lg cursor-pointer ${channel.id === progs.channelId
							...
							onClick={() => {
								...
								navigate(`/program/${progs.programId}/channel/${channel.id}`);
							}}
						>
							<p>{channel.name}</p>
						</div>
